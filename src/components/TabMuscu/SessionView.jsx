...
function computeSummary(exercises, todaySession, duration, getPR) {
  ...
  exercises.forEach(ex => {
    ...
    for (let i = 0; i < ex.sets; i++) {
      totalSetsTarget++;
      if (todaySession.sets[`${ex.id}_${i}`]) {
        totalSetsDone++;
        totalVolume += w * repsAvg;
      }
    }
...
function StatBox({ icon, label, value, sub, color }) {
  return (
    <div style={{
      background: 'var(--surface)', borderRadius: 'var(--r1)',
      padding: 'var(--s3)', textAlign: 'center',
    }}>
...
function SessionSummary({ summary }) {
  const pct = summary.totalSetsTarget > 0
    ? Math.round((summary.totalSetsDone / summary.totalSetsTarget) * 100)
    : 0;
  ...
  return (
    <div style={{
      ...
      {summary.prs.length > 0 && (
        <div style={{
          background: 'linear-gradient(90deg, #ff6f0022, #ffa00022)',
          border: '1px solid #ff6f0055',
          borderRadius: 'var(--r1)', padding: 'var(--s3)',
        }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#ff6f00', marginBottom: 4 }}>
            &#127942; Records personnels
          </div>
...
export default function SessionView({ workout }) {
  ...
  return (
    <div style={{ padding: 'var(--s4)', paddingBottom: 'calc(var(--tab-height) + env(safe-area-inset-bottom) + 80px)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--s2)', marginBottom: 'var(--s4)' }}>
        {Object.keys(WORKOUT_TYPES).filter(t => t !== 'Repos').map(type => {
          ...
          return (
            <button
              key={type}
              onClick={() => !sessionStarted && setSelectedType(type)}
              style={{
      ...
      {showFinish && (
        <div className="modal-overlay" onClick={() => setShowFinish(false)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}
            ...
            <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 'var(--s4)' }}>
              &#127881; Fin de seance
            </div>
      ...
      {/* Bouton flottant calculateur de disques */}
      <button
