...
function GuidedExercise({ exercise, swappedName, workout, sessionStarted, restTimer, onCycleAlt }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      <ExerciseImage exerciseId={exercise.id} exerciseName={displayName} swappedAlt={swappedName} hasAlts={hasAlts} tip={exercise.tips} showTip={showTip} onCycleAlt={onCycleAlt} onToggleTip={() => setShowTip(v => !v)} />
      ...
      <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--surface)', padding: 'var(--s3) var(--s4)', paddingBottom: 'max(var(--s3), env(safe-area-inset-bottom))', display: 'flex', gap: 'var(--s2)', alignItems: 'stretch' }}>
        ...
        <div style={{ flex: 1, padding: 'var(--s2) var(--s3)', borderRadius: 'var(--r2)', background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2, minWidth: 0 }}>
          <span style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Répétitions</span>
          <span style={{ fontWeight: 800, fontSize: 15 }}>{repsText} reps</span>
        </div>
